...
export default function InProfileSaver({inProfile, onNotification}) {
    ...
    const generateXML = (profile) => {
        ...
        // Material Type (für "Other" Material)
        if (profile.materialType !== undefined && profile.materialType !== null && profile.materialType !== '') {
            xml += `  <MaterialType>${escapeXML(String(profile.materialType))}</MaterialType>\n`;
        }
        ...
        // Flow stress parameters (verschachtelt in flowStressParams)
        if (profile.flowStressParams && typeof profile.flowStressParams === 'object') {
            const flowStressParams = ['a', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8', 'm9', 'baseStrain', 'baseStrainRate'];
            const hasFlowStressParams = flowStressParams.some(param =>
                profile.flowStressParams[param] !== undefined &&
                profile.flowStressParams[param] !== null &&
                profile.flowStressParams[param] !== ''
            );

            if (hasFlowStressParams) {
                xml += '  <FlowStressParameters>\n';
                flowStressParams.forEach(param => {
                    const value = profile.flowStressParams[param];
                    if (value !== undefined && value !== null && value !== '') {
                        // Konvertiere camelCase zu PascalCase für XML
                        const paramName = param === 'a' ? 'A' :
                            param === 'baseStrain' ? 'BaseStrain' :
                                param === 'baseStrainRate' ? 'BaseStrainRate' :
                                    param.toUpperCase(); // m1 -> M1, etc.
                        xml += `    <${paramName}>${escapeXML(String(value))}</${paramName}>\n`;
                    }
                });
                xml += '  </FlowStressParameters>\n';
            }
